...
public class ControllerCadFuncionario implements ActionListener {
    ...
    private boolean isFormularioValido() {
        ...
        boolean cpfPreenchido = utilities.Utilities.apenasNumeros(telaCadastroFuncionario.getjFormattedTextFieldCpf().getText()).length() > 0;
        if (!cpfPreenchido) {
            JOptionPane.showMessageDialog(null, "Preencha ao menos um dos campos: CPF.");
            telaCadastroFuncionario.getjFormattedTextFieldCpf().requestFocus();
            return false;
        }
        if (cpfPreenchido && !utilities.ValidadorCampos.validarCpf(telaCadastroFuncionario.getjFormattedTextFieldCpf().getText())) {
    ...
    private void handleGravar() {
        ...
        funcionario.setSexo(
            telaCadastroFuncionario.getjComboBoxSexo().getSelectedItem().equals("Masculino") ? 'M' : 'F'
        );
       
        funcionario.setStatus(
            telaCadastroFuncionario.getjComboBoxStatus().getSelectedItem().equals("Ativo") ? 'A' : 'I'
        );

        if (telaCadastroFuncionario.getjTextFieldId().getText().trim().isEmpty()) {
    ...
    private void handleBuscar() {
        ...
        if (codigo != 0) {
            ...
            telaCadastroFuncionario.getjFormattedTextFieldCpf().setText(utilities.Utilities.formatarCpf(funcionario.getCpf()));
